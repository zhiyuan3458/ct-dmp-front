<template>
  <div class="lzy-table-wrapper">
    ...
    <div ref="mapForm">
      <slope-map-form :bridgeInfo="metaData.mapBridgeInfo"></slope-map-form>
    </div>
  </div>
</template>
...
import slopeMapForm from './mapForm';
export default {
  components: {
    rightNavLayout,
    customTable,
  ...
  data ()
  {
    return {
      metaData: {
        ...
        popup: null, // 标记点弹窗
        position: [],
        mapBridgeInfo: null // 地图表格中展示的边坡信息
      }
    };
  },
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SLOPE/add', {deptId: this.deptId});
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SLOPE/show', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SLOPE/edit', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    /**
     * 定位
     * @author   gongjuncheng
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('roadNetworkAssetManagementSystem/_SLOPE/remove', {id: ids}).then(() =>
          {
            this.$store.dispatch('roadNetworkAssetManagementSystem/_SLOPE/listJson', {
              deptId: this.deptId,
