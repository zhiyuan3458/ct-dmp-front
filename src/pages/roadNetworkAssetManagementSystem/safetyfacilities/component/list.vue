<template>
  <div class="lzy-table-wrapper">
    ...
    <div ref="mapForm">
      <safetyfacilities-map-form :bridgeInfo="metaData.mapBridgeInfo"></safetyfacilities-map-form>
    </div>
  </div>
</template>
...
import safetyfacilitiesMapForm from './mapForm';
export default {
  components: {
    rightNavLayout,
    customTable,
    customDialog,
    safetyfacilitiesMapForm
  },
  data ()
  {
    return {
  ...
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SAFETYFACILITIES/add', {deptId: this.deptId});
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SAFETYFACILITIES/show', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_SAFETYFACILITIES/edit', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    /**
     * 定位
     * @author   gongjuncheng
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('roadNetworkAssetManagementSystem/_SAFETYFACILITIES/remove', {id: ids}).then(() =>
          {
            this.$store.dispatch('roadNetworkAssetManagementSystem/_SAFETYFACILITIES/listJson', {
              deptId: this.deptId,
