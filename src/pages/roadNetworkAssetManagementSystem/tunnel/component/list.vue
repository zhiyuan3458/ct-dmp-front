<template>
  <div class="lzy-table-wrapper">
    ...
    <div ref="mapForm">
      <tunnel-map-form :bridgeInfo="metaData.mapBridgeInfo"></tunnel-map-form>
    </div>
  </div>
</template>
...
import tunnelMapForm from './mapForm';
export default {
  components: {
    rightNavLayout,
    customTable,
  ...
  data ()
  {
    return {
      metaData: {
        ...
        popup: null, // 标记点弹窗
        position: [],
        mapBridgeInfo: null // 地图表格中展示的隧道信息
      }
    };
  },
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/add', {deptId: this.deptId});
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/show', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/edit', {id: row.id, deptId: this.deptId, userId: row.id});
    },
    getTableData ()
    {
      this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/listJson', {
        deptId: this.deptId,
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/remove', {id: ids}).then(() =>
          {
            this.$store.dispatch('roadNetworkAssetManagementSystem/_TUNNEL/listJson', {
              deptId: this.deptId,
              pageNum: this.metaData.currentPage,
