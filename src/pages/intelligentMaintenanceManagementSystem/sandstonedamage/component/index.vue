...
export default {
  ...
  methods: {
    /** initTree ()
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_SANDSTONEDAMAGE/getTree').then(data =>
      {
    ...
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_SANDSTONEDAMAGE/showList', treeNode.id);
    },
    // 点击侧导航栏的上部图标触发
    clickTopBtn (e)
    {
    ...
    submitForm ()
    {
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_SANDSTONEDAMAGE/showList', formCondition);
    }
  },
  computed: {
