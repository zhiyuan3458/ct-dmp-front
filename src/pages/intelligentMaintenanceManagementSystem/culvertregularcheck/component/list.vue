...
export default {
  ...
  methods: {
    ...
    addFn ()
    {
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTREGULARCHECK/add', {belongDeptId: this.deptIds});
    },
    /**
       * @Description: 查看
       * @author durenniu
    ...
    showFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTREGULARCHECK/show', {id: row.id});
    },
    /**
       * @Description: 编辑
       * @author durenniu
    ...
    editFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTREGULARCHECK/edit', {id: row.id, deptId: this.deptIds, userId: row.id});
    },
    /**
       * @Description: 删除
       * @author durenniu
    ...
    removeFn ()
    {
      ...
      {
        let culvertrRegularCheckIds = '';
        for (let row of this.multipleSelection)
        {
          culvertrRegularCheckIds += row.id + ',';
        }
        culvertrRegularCheckIds = culvertrRegularCheckIds.substring(0, culvertrRegularCheckIds.length - 1);
        ...
        {
          this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTREGULARCHECK/remove', {id: culvertrRegularCheckIds}).then(() =>
          {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    getTableData ()
    {
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTREGULARCHECK/listJson', {
