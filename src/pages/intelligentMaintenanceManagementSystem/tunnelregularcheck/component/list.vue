...
export default {
  ...
  methods: {
    addFn ()
    {
      ...
      {
        this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/add', {administrationDeptId: this.deptIds});
      }
    },
    editFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/edit', {id: row.id, deptId: this.deptIds, userId: row.id});
    },
    showFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/show', {id: row.id});
    },
    removeFn ()
    {
      ...
      else
      {
        ...
        }).then(() =>
        {
          ...
          this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/remove', {id: id.join(',')}).then(() =>
          {
            this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/listJson', {
    ...
    handleChangePosition (name)
    {
      ...
      if (preId)
      {
        ...
        this.$store.dispatch('intelligentMaintenanceManagementSystem/_TUNNELREGULARCHECK/changePosition', obj).then(() =>
        {
          this.$message({
            message: '修改顺序成功!',
            type: 'success',
