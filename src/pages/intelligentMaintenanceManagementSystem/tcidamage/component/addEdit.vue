  ...
  <div class="lzy-form-wrapper">
    ...
    <div class="form-container">
      <el-form :model="formData" ref="ctForm" v-loading="metaData.loading">
        <table class="form-table" width="100%">
          ...
          <tr>
            <th class="form-header" colspan="8">
              <h2 style="text-align: center">沿路设施损坏调查表</h2>
            </th>
          </tr>
          <tr>
            <th>路线编号</th>
        ...
        <inline-edit-table
          relatedTableCode="TciDamageRecord"
          :initDataInfo="listRecord"
          @change="listRecordChange"
          :deleteUrl="$prifix.maintenance + '/mt/ta/tcidamage/removeTciDamageRecord'"
          :isMinus = "true"
          :isAdd = "true"
        ></inline-edit-table>
        <table class="form-table" width="100%">
...
export default {
  ...
  methods: {
      ...
      }).then(() =>
      {
        this.$refs['ctForm'].validate((valid) =>
        {
          if (valid)
          {
            ...
            this.loading = true;
            this.$store.dispatch('intelligentMaintenanceManagementSystem/_TCIDAMAGE/save', obj);
          }
          else
          {
            return false;
    ...
    returnFn ()
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_TCIDAMAGE/showList', this.deptId);
    }
  },
  computed: {
