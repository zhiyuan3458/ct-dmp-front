...
export default {
  ...
  methods: {
    ...
    submitForm ()
    {
       ...
       * 在此处添加判断，如哪些条件是必须的
       */
      let formCondition = {deptId: this.deptId, month: this.month, year: this.year};
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_PROJECTMANAGEMENT/showList', formCondition);
    },
    /**
     * 初始化select树数据
     * @author   chenguang
    ...
    initSelectTree ()
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_PROJECTMANAGEMENT/getTree').then(data =>
      {
