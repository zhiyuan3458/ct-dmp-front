...
export default {
  ...
  methods: {
    ...
    submitForm ()
    {
       ...
       * 在此处添加判断，如哪些条件是必须的
       */
      let formCondition = {deptIds: this.deptIds, month: this.month, year: this.year, routeCode: this.routeCode, sectionName: this.sectionName};
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/showList', formCondition);
    },
    /**
     * 初始化select树数据
     * @author   chenguang
    ...
    initSelectTree ()
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/getTree').then(data =>
      {
        console.info(data);
