...
export default {
  ...
  methods: {
    ...
    addFn ()
    {
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/add', {belongDeptId: this.deptIds});
    },
    /**
       * @Description: 查看
       * @author durenniu
    ...
    showFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/show', {id: row.id});
    },
    /**
       * @Description: 编辑
       * @author durenniu
    ...
    editFn (row)
    {
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/edit', {id: row.id, deptId: this.deptIds, userId: row.id});
    },
    /**
       * @Description: 删除
       * @author durenniu
    ...
    removeFn ()
    {
      ...
      {
        let culvertOftenCheckIds = '';
        for (let row of this.multipleSelection)
        {
          culvertOftenCheckIds += row.id + ',';
        }
        culvertOftenCheckIds = culvertOftenCheckIds.substring(0, culvertOftenCheckIds.length - 1);
        ...
        {
          this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/remove', {id: culvertOftenCheckIds}).then(() =>
          {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    getTableData ()
    {
      ...
      this.$store.dispatch('intelligentMaintenanceManagementSystem/_CULVERTOFTENCHECK/listJson', {
