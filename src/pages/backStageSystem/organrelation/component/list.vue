  ...
  <div class="lzy-table-wrapper" v-if="isListShow" v-loading="isLoading">
    ...
    <div class="permission-tree-container">
      <tree-collapse-bar
        ...
        treeId="organrelationTree"
      >
        <!-- 树形结构工具栏 -->
        <ul slot="tree" id="organrelationTree" class="ztree"></ul>
      </tree-collapse-bar>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveFn ()
    {
      ...
      }).then(() =>
      {
        ...
        arr = arr.join(',');
        this.$store.dispatch('backStageSystem/_ORGANRELATION/save', { deptId: this.deptId, deptIds: arr });
      });
    }
  },
  computed: {
    ...mapGetters('backStageSystem/_ORGANRELATION', [
      'sideBar',
      'organrelationTreeList',
      'deptId',
      'isLoading',
      'isListShow',
      'operateBtns',
  ...
  watch: {
    ...
    organrelationTreeList (newValue, oldValue)
    {
      setTimeout(() =>
      {
        if (newValue.length !== 0)
        {
          this.metaData.treeTool = new TreeTool('organrelationTree', this.organrelationTreeList, true);
          this.metaData.treeTool.setChkboxType({ Y: 's', N: 's' });
