...
export default {
  ...
  methods: {
    ...
    initTree ()
    {
      this.$store.dispatch('backStageSystem/_APPMENU/getTree').then(data =>
      {
        let level = 1;
        ...
        this.metaData.treeTool.init('/api/backstage/sys/bs/appmenu/dragDrop', this.beforeDrop);
    ...
    {
      this.$store.dispatch('backStageSystem/_APPMENU/edit', {id: treeNode.id});
    },
    ...
    addFn (event)
    {
      ...
      this.$store.dispatch('backStageSystem/_APPMENU/add', {type: 0, parentId: treeNode.id});
    ...
    removeFn (event)
    {
      ...
      {
        this.$store.dispatch('backStageSystem/_APPMENU/remove', {
          type: treeNode.type,
          id: treeNode.id
        }).then(data =>
        {
  ...
  computed: {
    ...mapGetters('backStageSystem/_APPMENU', [
      'sideBar',
      'isRefreshTree',
      'treeList'
    ])
