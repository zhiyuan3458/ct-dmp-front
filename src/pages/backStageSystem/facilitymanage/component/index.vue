...
export default {
  ...
  methods: {
    ...
    initTree ()
    {
      this.$store.dispatch('backStageSystem/_FACILITYMANAGE/getTree').then(data =>
      {
        let level = 1;
        ...
        this.metaData.treeTool.init('/api/backstage/sys/facilitymanage/dragDrop', this.beforeDrop);
    ...
    {
      this.$store.dispatch('backStageSystem/_FACILITYMANAGE/edit', {id: treeNode.id});
    },
    ...
    addFn (event)
    {
      ...
      this.$store.dispatch('backStageSystem/_FACILITYMANAGE/add', {parentId: treeNode.id});
    ...
    {
      let treeNode = this.metaData.treeTool.getSelectNode(event);
      this.$confirm(`将会删除公路设施管理下的所有数据!\t确认删除“${treeNode.text}”?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() =>
      {
        this.$store.dispatch('backStageSystem/_FACILITYMANAGE/remove', {
          type: treeNode.type,
          id: treeNode.id
        }).then(data =>
        {
