<template>
  <div>
    <custom-table
      height="255px"
      v-loading="metaData.isFutureLoading"
      :tableList="futureList"
      :gridEntityInfo="futureGridEntityInfo"
      :tableBtns="tableBtns"
      :listGridColumnInfo="futureListGridColumnInfo"
    ></custom-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    showFn (row)
    {
      this.$store.dispatch('backStageSystem/_WEATHERFORECAST/futureShow', {id: row.id});
    },
    getTableData ()
    {
      this.metaData.isFutureLoading = true;
      this.$store.dispatch('backStageSystem/_WEATHERFORECAST/futureListJson', {
        cityId: this.cityId,
      ...
      }).then(() =>
      {
        ...
        this.metaData.isFutureLoading = false;
      });
    },
    /**
     * 复选框选定
  ...
  computed: {
    ...mapGetters('backStageSystem/_WEATHERFORECAST', [
      ...
      'tableBtns',
      'navigateMenus',
      'futureGridEntityInfo',
      'futureListGridColumnInfo'
    ]),
    opened ()
    {
