...
export default {
  ...
  methods: {
    ...
    initTree ()
    {
      this.$store.dispatch('backStageSystem/_WEATHERFORECAST/getTree').then(data =>
      {
    ...
    clickFn (event, treeId, treeNode)
    {
      if (treeNode.level === 0)
      {
        this.$message('请选择其它节点!');
      }
      else
      {
        this.$store.dispatch('backStageSystem/_WEATHERFORECAST/showList', { cityId: treeNode.code });
      }
    }
  },
  /**
