...
import { getrouter } from '@/api/mainFrame/header';
export default {
  components: {
    rightNavLayout,
    customTable,
  ...
  methods: {
    addFn ()
    {
      this.$store.dispatch('backStageSystem/_TODOTASK/add', '');
    },
    auditFn (row)
    {
      if (row.userClick === '已处理')
      {
        this.$message('任务已处理!');
        return;
      }
      let obj = { processInstanceId: row.processInstanceId };
    ...
    transferFn (row)
    {
      ...
      this.$store.dispatch('backStageSystem/_TODOTASK/transfer', { processInstanceId: row.processInstanceId });
    },
    getTableData ()
    {
      this.$store.dispatch('backStageSystem/_TODOTASK/listJson', {
