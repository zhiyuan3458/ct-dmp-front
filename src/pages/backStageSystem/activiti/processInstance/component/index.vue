...
export default {
  ...
  methods: {
    ...
    initTree ()
    {
      this.$store.dispatch('backStageSystem/_PROCESSINSTANCE/getTree').then(data =>
      {
    ...
    clickFn (event, treeId, treeNode)
    {
      ...
      {
        let obj = {value: treeNode.text, id: treeNode.id, refresh: new Date()};
        this.$store.dispatch('backStageSystem/_PROCESSINSTANCE/showList', obj);
      }
    }
  },
  created ()
