...
export default {
  ...
  methods: {
    ...
    {
      let level = 3;
      this.$store.dispatch('backStageSystem/_APPPERMISSION/getRoleTree').then(data =>
      {
    ...
    clickFn (event, treeId, treeNode)
    {
      ...
      {
        this.$store.dispatch('backStageSystem/_APPPERMISSION/getTree', {roleId: treeNode.id});
      }
    }
  },
  computed: {
    ...mapGetters('backStageSystem/_APPPERMISSION', [
      'sideBar'
    ]),
    opened ()
    {
