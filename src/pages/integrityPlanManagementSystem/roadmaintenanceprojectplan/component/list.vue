...
export default {
  ...
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_ROADMAINTENANCEPROJECTPLAN/add');
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_ROADMAINTENANCEPROJECTPLAN/show', { id: row.id });
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_ROADMAINTENANCEPROJECTPLAN/edit', { id: row.id });
    },
    getTableData ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_ROADMAINTENANCEPROJECTPLAN/listJson', {
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('integrityPlanManagementSystem/_ROADMAINTENANCEPROJECTPLAN/remove', {ids: ids}).then(() =>
          {
            this.$message({
              type: 'success',
              message: '删除成功!'
