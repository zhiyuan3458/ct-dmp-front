...
export default {
  ...
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_MAINTENANCELINEGRADING/add', { routeCode: this.mlgParams.routeCode });
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_MAINTENANCELINEGRADING/show', { id: row.id });
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_MAINTENANCELINEGRADING/edit', { id: row.id });
    },
    getTableData ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_MAINTENANCELINEGRADING/listJson', {
        extraCondition: this.mlgParams.extraCondition,
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('integrityPlanManagementSystem/_MAINTENANCELINEGRADING/remove', {ids: ids}).then(() =>
          {
            this.$message({
              type: 'success',
              message: '删除成功!'
