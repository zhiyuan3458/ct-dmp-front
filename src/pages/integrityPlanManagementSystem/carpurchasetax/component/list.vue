...
export default {
  ...
  methods: {
    ...
    addFn ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_CARPURCHASETAX/add', { planAffiliation: this.carPurchaseTax.planAffiliation });
    },
    /**
     * 查看
     * @author   chenguang
    ...
    showFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_CARPURCHASETAX/show', { id: row.id });
    },
    /**
     * 编辑
     * @author   chenguang
    ...
    editFn (row)
    {
      this.$store.dispatch('integrityPlanManagementSystem/_CARPURCHASETAX/edit', { id: row.id });
    },
    getTableData ()
    {
      this.$store.dispatch('integrityPlanManagementSystem/_CARPURCHASETAX/listJson', {
        extraCondition: this.carPurchaseTax.extraCondition,
    ...
    removeFn ()
    {
      if (this.multipleSelection !== undefined && this.multipleSelection.length > 0)
      {
        ...
        {
          this.$store.dispatch('integrityPlanManagementSystem/_CARPURCHASETAX/remove', {ids: ids}).then(() =>
          {
            this.$message({
              type: 'success',
              message: '删除成功!'
