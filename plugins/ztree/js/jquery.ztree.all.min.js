(function($) {
  var settings = {},
    ...
    _initNode = function(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
      ...
      if (tools.isArray(children) && !(isParent === false || (typeof isParent == "string" && tools.eqs(isParent, "false")))) {
    ...
    view = {
      ...
      expandCollapseSonNode: function(setting, node, expandFlag, animateFlag, callback) {
        ...
        if (treeNodes) {
          for (var i = 0, l = treeNodes.length;
               i < l; i++) {
            if (treeNodes[i]) {
    ...
    _zTreeTools = function(setting, zTreeTools) {
      zTreeTools.checkNode = function(node, checked, checkTypeFlag, callbackFlag) {
        ...
        if (nodeChecked === checked && !checkTypeFlag) {
          ...
          if (callbackFlag && tools.apply(this.setting.callback.beforeCheck, [this.setting.treeId, node], true) == false) {
            return
          }
        }
        ...
        if (children) {
          for (var i = 0, l = children.length; i < l; i++) {
            ...
            if (setting.check.chkStyle == consts.radio.STYLE) {
              ...
              if (setting.check.chkStyle == consts.checkbox.STYLE) {
                if (cNode.nocheck === true || cNode.chkDisabled === true) {
                  tmp = cNode.check_Child_State
                } else {
                  if (cNode.halfCheck === true) {
                ...
                if (tmp === 1) {
                  ...
                  if (tmp === 2 && chkFlag > -1 && i > 0 && tmp !== chkFlag) {
                    chkFlag = 1;
                    break
                  } else {
                    if (chkFlag === 2 && tmp > -1 && tmp < 2) {
                      chkFlag = 1;
                      break
                    } else {
                      if (tmp > -1) {
      ...
      setSonNodeCheckBox: function(setting, node, value, srcNode) {
        ...
        if (node != data.getRoot(setting) && node.chkDisabled !== true) {
          ...
          if (node.nocheck !== true && node.chkDisabled !== true) {
            data.nodeChecked(setting, node, value);
            if (!hasDisable) {
              node.check_Child_State = (children && children.length > 0) ? (value ? 2 : 0) : -1
            }
          } else {
            ...
            if (tmpTarget && tmpTarget.attr("id") != targetSetting.treeId) {
              ...
              if (!canPrev && !canNext && !canInner) {
                ...
                if ((prevPercent == 1 || dY_percent <= prevPercent && dY_percent >= -0.2) && canPrev) {
                  ...
                  if ((nextPercent == 0 || dY_percent >= nextPercent && dY_percent <= 1.2) && canNext) {
                    dX = 1 - tmpArrow.width();
    ...
    _view = {
      ...
      moveNode: function(setting, targetNode, node, moveType, animateFlag, isSilent) {
        ...
        if (target_ulObj.get(0) && moveType == consts.move.TYPE_INNER) {
          ...
          if (targetObj.get(0) && moveType == consts.move.TYPE_PREV) {
            targetObj.before(nodeDom)
          } else {
            if (targetObj.get(0) && moveType == consts.move.TYPE_NEXT) {
        ...
        if (moveType == consts.move.TYPE_INNER) {
          ...
          if (targetNode.isFirstNode && moveType == consts.move.TYPE_PREV) {
            ...
            if (targetNode.isLastNode && moveType == consts.move.TYPE_NEXT) {
